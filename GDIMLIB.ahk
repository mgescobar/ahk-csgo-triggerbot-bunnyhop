class GDI
{
	__New(hWnd, CliWidth=0, CliHeight=0,Backgroundcolor=0x000000)
	{
		...
		this.Ret := DllCall("SetGraphicsMode","Ptr", this.hDC,"Int", 0x1)
		this.hMemDC := DllCall("CreateCompatibleDC", "UPtr", this.hDC, "UPtr"),
		this.hBitmap := DllCall("CreateCompatibleBitmap","UPtr", this.hDC, "Int", this.CliWidth, "Int", this.CliHeight, "UPtr"),
		this.hOriginalBitmap := DllCall("SelectObject", "UPtr", this.hMemDC, "UPtr", this.hBitmap)
		this.Background := new this.Brush(Backgroundcolor)
	}
	...
	BitBlt(x=0, y=0, w=0, h=0)
	{
		...
		DllCall("BitBlt", "UPtr", this.hDC, "Int", x, "Int", y, "Int", w, "Int", h, "UPtr", this.hMemDC, "Int", 0, "Int", 0, "UInt", 0x00C000CA) , DllCall("SelectObject", "UPtr", this.hMemDC, "UPtr", this.hOriginalBitmap), DllCall("DeleteObject", "UPtr", this.hBitmap),       	DllCall("DeleteObject", "UPtr", this.hMemDC)
	}
	...
	DrawLinePen(x, y, x2, y2, Pen)
	{
		static OldPen
		if (OldPen.Handle != Pen.Handle)
		{
			OldPen.Handle := Pen.Handle
			DllCall("SelectObject", "UPtr", this.hMemDC, "UPtr", Pen.Handle, "UPtr")
		}
		DllCall("MoveToEx", "UPtr", this.hMemDC, "Int", this.TranslateX(x), "Int", this.TranslateY(y), "UPtr", 0)
		;hOriginalPen := DllCall("SelectObject", "UPtr", this.hMemDC, "UPtr", Pen.Handle, "UPtr")
		DllCall("LineTo", "UPtr", this.hMemDC, "Int", this.TranslateX(x2), "Int", this.TranslateY(y2))
	...
	DrawTextTest(x, y, w, h, Text, Color, Font, Align="LT")
	{
		AlignH := {"L": 0x00, "C": 0x01, "R": 0x02}
		AlignV := {"C": 0x24, "B": 0x28, "T": 0x00}
		Align := StrSplit(Align)
	...
	DrawRectangle(x, y, w, h, Pen)
	{
		; Replace the original pen and brush with our own
		hOriginalPen := DllCall("SelectObject", "UPtr", this.hMemDC, "UPtr", Pen.Handle, "UPtr"), hOriginalBrush := DllCall("SelectObject", "UPtr", this.hMemDC, "UPtr", this.Background.Handle, "UPtr"), x1 := this.TranslateX(x), x2 := this.TranslateX(x+w), y1 := this.TranslateY(y), y2 := this.TranslateY(y+h)
		
		DllCall("Rectangle", "UPtr", this.hMemDC, "Int", x1, "Int", y1, "Int", x2, "Int", y2)
...
class _ClassMemory
{
    ...
    ;       VoM     -   Default, Vector, Matrix.
    ;                   Vector =  Vector3, Matrix Will Return a 4x4 Matrix.
    ...
    read(address, type := "UInt",VoM := "Default", aOffsets*)
    {
        ...
        if !this.aTypeSize.hasKey(type)
            return "Wrong Type!", ErrorLevel := -2 
        if (VoM == "Default")
        {
            if DllCall("ReadProcessMemory", "Ptr", this.hProcess, "Ptr", aOffsets.maxIndex() ? this.getAddressFromOffsets(address, aOffsets*) : address, type "*", result, "Uint", this.aTypeSize[type], "Ptr",0) 
                return result
            Return "0"
        }            
        if (VoM == "Vector")
        {
            VarSetCapacity(result,12,0)
            if DllCall("ReadProcessMemory", "Ptr", this.hProcess, "Ptr", aOffsets.maxIndex() ? this.getAddressFromOffsets(address, aOffsets*) : address, "ptr", &result, "Uint", this.aTypeSize[type]*3, "Ptr",0)
            {
                if !IsObject(Vector)
                    Vector := Object()
                Vector := [ NumGet(result,0,"Float"), NumGet(result,4,"Float"), NumGet(result,8,"Float")]
                return Vector
            }
            Vector := [0,0,0]
            return Vector 
        ...
        if (VoM == "Matrix")
        {
            if DllCall("ReadProcessMemory", "Ptr", this.hProcess, "Ptr", aOffsets.maxIndex() ? this.getAddressFromOffsets(address, aOffsets*) : address, "Ptr", &result, "Uint", this.aTypeSize[type]*16, "Ptr",0)
            {
                if !isObject(Matrix)
                    Matrix := Object()
                MRI := 0, Offset := 0
                Loop, 4 
                {
                    MRI ++, MCI = 1 , Matrix[MRI,MCI] := NumGet(result,Offset,"Float"), Offset += 4
                    Loop % 3 
                        {
                            MCI := A_Index + 1, Matrix[MRI,MCI] := NumGet(result,Offset,"Float"), Offset += 4
                        }
                }
                return Matrix
            }
    ...
    ;                   e.g. for calc.exe it would be the same as calling getModuleBaseAddress() or getModuleBaseAddress("calc.exe")
    ...
    */  
    ...
    ;       
    ; Parameters:
    ;   hexString -     A string of hex bytes.  The '0x' he prefix is optional.
